  ...
  def process_message(step)
    ...
    when 2
      ...
      if subscription.is_a?(Hash)
        ...
        message = "Your next delivery date is #{DateTime.parse(subscription.next_billing_date).strftime("%a, %B %e")}, Reply with (2w/1m/6w) if you want to skip?"
      end
    when 3
      ...
      if subscription_message.present?
        ...
        if subscription.is_a?(Hash)
          ...
          if next_scheule_date == false
            ...
            message = "Your next delivery date would be around the #{next_scheule_date.to_date.strftime("%a, %B %e")}, are you sure you want to proceed?"
          end
        end
      else
        error = true
