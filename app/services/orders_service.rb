class OrdersService
  ...
  def orders_in_range(range_start, range_end, fields = nil)
    ...
    while orders.length == limit
      ...
      orders = ShopifyAPIRetry::REST.retry { ShopifyAPI::Order.find(:all, params: { status: 'any', limit: limit, created_at_min: range_start, created_at_max: range_end, since_id: since_id, fields: fields }) }
      all_orders = all_orders.concat orders
    end
    all_orders
  rescue Exception => ex
