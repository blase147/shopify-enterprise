      ...
      def resolve(**args)
        if current_user.present?
            if args[:searchquery].present?
                staff_members = UserShopChild.where(user_shop_id: current_user&.user_shop.id).joins(:user).search(args[:searchquery]).page(args[:page])
            else
                staff_members = UserShopChild.where(user_shop_id: current_user&.user_shop.id).page(args[:page])
            end
