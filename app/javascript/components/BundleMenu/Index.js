...
const BundleMenus = ({ shopifyDomain }) => {
  ...
  useEffect(() => {
    setTotalPages(data?.fetchBundles?.totalPages)
    if (+page < +totalPages) {
      setHasMore(true);
    }
    else {
  ...
  useEffect(() => {
    if (data && data.fetchBundles) {
      let rowsData = formatRows(data.fetchBundles?.bundles);
      setCustomers(data.fetchBundles?.bundles);
    ...
    <>
      <Frame>
        ...
        <Page
          ...
          primaryAction={
            <ButtonGroup>
              <Button onClick={() => history.push("/createBundleMenu")}>
                Create Bundle Menu
              </Button>
              {/*<Button
                onClick={() => {
