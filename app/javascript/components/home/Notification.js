...
const [fetchNotifications,{loading,data:notifications,error}]=useLazyQuery(notificationQuery,{fetchPolicy:"network-only"});
  ...
  useEffect(()=>{
    console.log("hehehe",notifications?.fetchSubscriptionLogs?.totalPages)
    if(notifications?.fetchSubscriptionLogs?.totalPages){
      console.log("2hehehe",notifications?.fetchSubscriptionLogs?.totalPages)
          ...
          >
            <>
              <Card title="Live Notifications" sectioned>
                {
                  console.log(notifications?.fetchSubscriptionLogs?.subscriptionLogs,"----logs",hasMore,page,totalPages)
                }
                {
