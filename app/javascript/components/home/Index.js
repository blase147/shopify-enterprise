  ...
  useEffect(() => {
    if (!isEmpty(data?.fetchDashboardReport)) {
      ...
      setSectionListData((prevSecList) => ({
        ...
        subscriptions:
          fetchDashboardReport.activeSubscriptionsCount ||
          prevSecList.subscriptions,
        churn_rate: fetchDashboardReport.churnRate || prevSecList.churn_rate,
        cl_value:
          fetchDashboardReport.customerLifetimeValue || prevSecList.cl_value,
      }));
      ...
      const newSale = {
        ...
        series: [
          {
            data:
              fetchDashboardReport?.salesData.map((item) =>
                parseInt(item.data.value)
              ) || [],
          },
        ],
        xAxis:
          {
            ...sale.xAxis,
            categories: fetchDashboardReport?.salesData.map(
              (item) => item.date
            ),
          } || [],
      };
      const newCustomer = {
        ...
        series: [
          {
            data:
              fetchDashboardReport?.activeCustomers.map((item) =>
                parseInt(item.data.value)
              ) || [],
          },
        ],
        xAxis: {
          ...customer.xAxis,
          categories:
            fetchDashboardReport?.activeCustomers.map((item) => item.date) ||
            [],
        },
      };
      const newRevenueChurn = {
        ...revenue_churn,
        series: [
          {
            data:
              fetchDashboardReport?.revenueChurn.map((item) =>
                parseInt(item.data.value)
              ) || [],
          },
        ],
        xAxis: {
          ...revenue_churn.xAxis,
          categories:
            fetchDashboardReport?.revenueChurn.map((item) => item.date) || [],
        },
      };
      const newRenewal = {
        ...renewal_rate,
        series: [
          {
            data:
              fetchDashboardReport?.renewalData.map((item) =>
                parseInt(item.data.value)
              ) || [],
          },
        ],
        xAxis: {
          ...renewal_rate.xAxis,
          categories:
            fetchDashboardReport?.renewalData.map((item) => item.date) || [],
        },
      };
      const newMMR = {
        ...mrr,
        series: [
          {
            data:
              fetchDashboardReport?.mrrData.map((item) =>
                parseInt(item.data.value)
              ) || [],
          },
        ],
        xAxis: {
          ...mrr.xAxis,
          categories:
            fetchDashboardReport?.mrrData.map((item) => item.date) || [],
        },
      };
      const newARR = {
        ...arr,
        series: [
          {
            data:
              fetchDashboardReport?.arrData.map((item) =>
                parseInt(item.data.value)
              ) || [],
          },
        ],
        xAxis: {
          ...arr.xAxis,
          categories:
            fetchDashboardReport?.arrData.map((item) => item.date) || [],
        },
      };
      const newRefunds = {
        ...refunds,
        series: [
          {
            data:
              fetchDashboardReport?.refundData.map((item) =>
                parseInt(item.data.value)
              ) || [],
          },
        ],
        xAxis: {
          ...refunds.xAxis,
          categories:
            fetchDashboardReport?.refundData.map((item) => item.date) || [],
        },
      };
      const newCMRR = {
        ...cmrr,
      ...
      setChartSums({
        ...chartSums,
        sale: `$${fetchDashboardReport.salesData
          .map((item) => parseInt(item.data.value))
          .reduce((a, b) => a + b, 0)}`,
        customer: `${fetchDashboardReport.activeCustomers
          .map((item) => parseInt(item.data.value))
          .reduce((a, b) => a + b, 0)}`,
        ...
        arr: `$${fetchDashboardReport.arrData
          .map((item) => parseInt(item.data.value))
          .reduce((a, b) => a + b, 0)}`,
        refunds: `$${fetchDashboardReport.refundData
          .map((item) => parseInt(item.data.value))
          .reduce((a, b) => a + b, 0)}`,
        cmrr: `$${fetchDashboardReport.salesData
          .map((item) => parseInt(item.data.value))
          .reduce((a, b) => a + b, 0)}`,
      });
    }
  ...
  const sectionList = [
    {
      ...
      key: 'mrr',
      type: 'currency',
      info: 'Sum of all subscription revenue in a 30 day period. How to calculate MRR. if there are 10 active customers due to pay for subscriptions in June, the MRR is the total sum of new customers month to date and the active subscribers due for payment.',
    },
    {
      section: 'Active Subscriptions',
      amount: '00',
      key: 'subscriptions',
      info: 'Total number of subscriptions. For example, if a customer has 2 different subscriptions, it will be counted twice.',
    },
    {
      section: 'Churn Rate',
      amount: '0',
      key: 'churn_rate',
      type: 'percent',
      info: 'To calculate, compute the total number of customers that you lost in a period divided by the total number of customers at the beginning of that period.',
    },
    {
      ...
      key: 'cl_value',
      type: 'currency',
      info: 'The simplest model to calculate CLV is to divide the average monthly recurring revenue per customer by the monthly churn rate. So average monthly recurring revenue per customer = MRR/active customers',
    },
                              ...
                              <Stack>
                                ...
                                <Stack.Item>
                                  ...
                                  >
                                    ...
                                    />
                                    {Math.abs(
                                      sectionListData[item.key]?.percent
                                    ) || 0}
                                    %
                                  </TextStyle>
                                </Stack.Item>
                    ...
                    >
                      <div className="dashboard-bottom-card-section">
                        <Card>
                          <Card.Section>
                            <Heading>
                              {chartSums[item.tabs[item.selected]?.id]}
                            </Heading>
