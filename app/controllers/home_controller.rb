...
class HomeController < ApplicationController 
  ...
  def index
      unless session[:shop_id] # ensure cookie session as well
        session[:shop_id] = Shop.find_by(shopify_domain: get_user_shopify_domain)&.id
      end
      #if ENV['APP_TYPE'] == 'public' && current_shop.recurring_charge_id.blank?
       # redirect_to select_plan_index_path
      #else
        if current_shopify_domain.present?
          @shop_origin = current_shopify_domain
        else 
          @shop_origin = get_user_shopify_domain
        end
  ...
  def current_shop
    unless params[:hmac].present? && params[:shop].present?
      if session[:shop_domain].present?
        @current_shop = Shop.find_by(shopify_domain: session[:shop_domain])
      else
        @current_shop ||= Shop.find_by(shopify_domain: get_user_shopify_domain)
      end
    end
  end

  def get_user_shopify_domain
    return if params[:hmac].present? && params[:shop].present?
      current_shopify_domain ||= (current_user.user_shop&.shops&.first&.shopify_domain if current_user.present?)
  end

  def install_shop
    unless params[:hmac].present?
